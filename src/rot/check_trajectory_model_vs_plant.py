...
if __name__ == '__main__':
    ...
    for file in os.listdir(dirpath):
        ...
        t = np.linspace(t_start, 18, 10000)
        theta = Ftheta(t)
        dtheta = Fdtheta(t)
        alpha = Falpha(t)
        ...
        pers.append(per)
        break
    plt.plot(theta, linewidth = 4, label='experiment')
    ...
    plt.plot(theta_sim, linewidth=3, label='simulation')
    plt.grid()
    plt.legend()
    plt.title('theta')
    # plt.ylabel('dtheta')
    ...
    plt.plot(dtheta, linewidth=4, label='experiment')
    plt.plot(dtheta_sim, linewidth=3, label='simulation')
    plt.grid()
    plt.legend()
    plt.title('dtheta')
    # plt.ylabel('dtheta')
